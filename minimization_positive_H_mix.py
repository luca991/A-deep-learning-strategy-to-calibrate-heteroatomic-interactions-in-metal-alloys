...
def obj_function(var, fix):
    ...
    y_true = fix[11]
    fix_var = fix_tempo[:10] + [fix_tempo[10]] + var[:] + fix_tempo[15:17]
    x_data = pd.DataFrame([fix_var], columns=["A_00", "A_11", "q_00", "q_11", "xi_00", "xi_11", "p_00", "p_11", "r0_00", "r0_11", "T", "r0_01", "q_01", "p_01", "xi_01","A_01", "LS_0", "LS_1"])
    y_pred = Hmix5.predict(x_data, verbose=0)
    loss_1 = abs(y_pred[0][0]-y_true)/y_true
    print(y_true, "\t", y_pred[0][0])
    ...
    fix_var = fix_tempo[:10] + [fix_tempo[10]] + var[:] + fix_tempo[15:17]
    x_data = pd.DataFrame([fix_var], columns=["A_00", "A_11", "q_00", "q_11", "xi_00", "xi_11", "p_00", "p_11", "r0_00", "r0_11", "T", "r0_01", "q_01", "p_01", "xi_01","A_01", "LS_0", "LS_1"])
    y_pred = Hmix95.predict(x_data, verbose=0)
    loss_2 = abs(y_pred[0][0]-y_true)/y_true
    print(y_true, "\t", y_pred[0][0])
    print("difference: ", loss_2, "\n")

    print('latt_5 prediction')
    fix_tempo = remove_nth_el(fix, 14)
    y_true = fix[14]
    fix_var = fix_tempo[:10] + [fix_tempo[13]] + var[:] + fix_tempo[15:17]
    x_data = pd.DataFrame([fix_var], columns=["A_00", "A_11", "q_00", "q_11", "xi_00", "xi_11", "p_00", "p_11", "r0_00", "r0_11", "T", "r0_01", "q_01", "p_01", "xi_01","A_01", "LS_0", "LS_1"])
    y_pred = latt_5.predict(x_data, verbose=0)
    loss_3 = abs(y_pred[0][0]-y_true)/y_true
    print(y_true, "\t", y_pred[0][0])
    ...
    fix_var = fix_tempo[:10] + [fix_tempo[13]] + var[:] + fix_tempo[15:17]
    x_data = pd.DataFrame([fix_var], columns=["A_00", "A_11", "q_00", "q_11", "xi_00", "xi_11", "p_00", "p_11", "r0_00", "r0_11", "T", "r0_01", "q_01", "p_01", "xi_01","A_01", "LS_0", "LS_1"])
    y_pred = latt_95.predict(x_data, verbose=0)
    loss_4 = abs(y_pred[0][0]-y_true)/y_true
    print(y_true, "\t", y_pred[0][0])
    print("difference: ", loss_4, "\n")

    loss = (((2*(loss_1)**2 + 2*(loss_2)**2 + loss_3**2 + loss_4**2)/6)**0.5)
...
quanties=['A_00', 'A_11', 'q_00', 'q_11', 'xi_00', 'xi_11', 'p_00', 'p_11', 'r0_00', 'r0_11', 'T', 'H_mix_5', 'H_mix_95', 'T_latt_par', "latt_par_5", "latt_par_95", "LS_0", "LS_1", 'r0_01', 'q_01', 'p_01', 'xi_01', 'A_01']
